...
public class Main implements MainTemplateV2.RunJob<Configuration> {
    ...
    void internalRun(StreamExecutionEnvironment env, Configuration configuration, DataStream<StringObjectMap> inputStream, Class<Configuration> aClass) {
        ...
        FlinkKafkaProducer<StringObjectMap> sink = KafkaSourceHelper.flink1_12_2_KafkaSink(
                KafkaSourceHelper.KafkaSinkConfig.builder()
                        .brokers(kafkaSinkConfig.getBroker())
                        .topic(kafkaSinkConfig.getTopic())
                        .build(),
                new DroolBasedKeyFinder(ruleEngineProvider, configuration),
                StringObjectMap.class
        );
        processedStream.addSink(sink)
                .name("kafka-sink").uid("d169059a-3524-11ed-a261-0242ac120002");
